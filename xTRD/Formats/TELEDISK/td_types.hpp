...
const WORD N = 4096;	// buffer size
const WORD F = 60;	// lookahead buffer size
const WORD THRESHOLD =	2;
const WORD NIL = N;	// leaf of tree

BYTE text_buf[N + F - 1];

const WORD N_CHAR = (256 - THRESHOLD + F);  	// kinds of characters (character code = 0..N_CHAR-1)
const WORD T =	(N_CHAR * 2 - 1);	// size of table
const WORD R = (T - 1);			// position of root
const WORD MAX_FREQ = 0x8000;		// updates tree when the
